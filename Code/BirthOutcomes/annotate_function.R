...
# writes a text file of annotated results 

annotate <- function(outcome){
  
  for(i in 1:22){
    
    # read in chromosome results 
    file.name = paste0("Chr", i, "_results_", outcome, ".txt")
    ...
    write.table(re, paste0('Chr', i, '_annotated_', outcome, '.txt'), row.names=F, quote=F, sep = '\t')
